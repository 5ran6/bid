...
contract bidaVesting {
    ...
    event tenPercentEmit(address indexed sender, uint256 amount, uint256 time);
     ...
     constructor( address _bidaToken, address superUser) { 
        currentSaleState = true;
        claimFirstTenPercent = false;
        bidaToken = _bidaToken;
        sUser = superUser ; // 0x27eB67ACbE0f365E8Ef809FEB311101aBf129E61;
    }
    ...
    function migratedAccount(address[] memory mUser, uint256[] memory amountStaked) external onlyOwner {
        require(mUser.length == amountStaked.length, "Invalid Length....");
        for (uint256 i = 0; i <= mUser.length; i++) {
            UserData storage usd = userData[mUser[i]];
            usd.user = mUser[i];
            usd.amountLocked = amountStaked[i];
    ...
    function tenPercentClaim() external {
        ...
        usd.amountClaimed = usd.amountClaimed.add(tenPercent);
        c.firstClaim = true;
        totalAmountShared = totalAmountShared.add(tenPercent);
        IERC20(bidaToken).transfer(_msgSender(), tenPercent);
        emit tenPercentEmit(msg.sender, tenPercent, block.timestamp);
    }
